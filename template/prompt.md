# Nexus Test Plan Generator (Full-Stack Web Application)**
...
- Form fields: JIRA Base URL (e.g., `https://company.atlassian.net`), Username/Email, API Token (encrypted storage) - *[use the credentials from the .env file]*
- "Test Connection" button that validates credentials by fetching user info - *[use the credentials from the .env file]*
...
- **Groq Configuration:** API Key input, Model selection dropdown (llama3-70b, mixtral-8x7b, etc.), Temperature slider (0-1) - *[use the credentials from the .env file]*
- **Ollama Configuration:** Base URL input (default: `http://localhost:11434`), Model selection (fetch available models from `/api/tags`) - *[use the credentials from the .env file]*
- Toggle switch: "Use Local LLM (Ollama)" vs "Use Cloud LLM (Groq)"
- Validation: Test connection buttons for both providers - *[use the credentials from the .env file]*
...
**Step 1: Ticket Input**
- Clean input field for JIRA ID (e.g., "SCRUM-7") with validation
...
POST /api/jira/fetch           // Body: {ticketId: "SCRUM-7"}
...
- **Keyboard Shortcuts:** Ctrl+Enter to generate, Ctrl+Shift+S to save
  ...
  JIRA_BASE_URL= https://ai-vault.atlassian.net/
  JIRA_API_TOKEN= ATATT3xFfGF0h61iNTTfF-cCsznsnzhtQl-BoxVuQgTWs1_8v7uePbnQLtXWpiEbokatHXdFv8loYQZYFkD0V3TZALVXbVQSDiSqL88451jSPSAOo_O2G-Err-lCV9plzylioMMF2UL8UcSiJZKBEScbbQAJeP4quXjMoJpUXQQtUUZgbJN86Go=0B5AFFC7
  GROQ_API_KEY= gsk_3014212345678901234567890123456789012345678901234567890123456789
